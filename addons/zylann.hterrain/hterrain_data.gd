...
func resize(p_res: int, stretch := true, anchor := Vector2(-1, -1)):
	...
	for channel in CHANNEL_COUNT:
		# DEFENSIVE: Ensure _maps has entry for this channel (Fix for index 6 crash)
		while channel >= len(_maps):
			_maps.append([])
...
func notify_region_change(
	...
	# DEFENSIVE: Fix for index 6 crash
	while p_map_type >= len(_maps) or p_index >= len(_maps[p_map_type]):
		_logger.debug(str("Expanding channels for notify_region_change: type=", p_map_type))
		_edit_add_map(p_map_type)
...
func _upload_region(channel: int, index: int, min_x: int, min_y: int, size_x: int, size_y: int):
	...
	# DEFENSIVE: Fix for index 6 crash
	while channel >= len(_maps) or index >= len(_maps[channel]):
		_logger.debug(str("Expanding channels for _upload_region: type=", channel))
		_edit_add_map(channel)
...
func get_image(map_type: int, index := 0) -> Image:
	# DEFENSIVE: Fix for index 6 crash
	while map_type >= len(_maps) or index >= len(_maps[map_type]):
		_logger.debug(str("Expanding channels for get_image: type=", map_type))
		_edit_add_map(map_type)
...
func get_texture(map_type: int, index := 0, writable := false) -> Texture:
	...
	# DEFENSIVE: Fix for index 6 crash
	while map_type >= len(_maps) or index >= len(_maps[map_type]):
		_logger.debug(str("Expanding channels for get_texture: type=", map_type))
		_edit_add_map(map_type)
